...
module.exports = {
  ...
  capture: async (req, res) => {
    let transaction = await Transaction.findOne({ payment: req.params.id, type: 'authorization' });
    request = new paypal.payments.AuthorizationsCaptureRequest(transaction.external_reference);
    request.requestBody({});
    ...
    await Payment.findByIdAndUpdate(req.params.id, { status: 'capture' });
  ...
  refund: async (req, res) => {
    ...
    await Payment.findByIdAndUpdate(req.params.id, { status: 'refund' });
    console.log(request);
    console.log(response);
    res.status(response.statusCode)
    .send({
